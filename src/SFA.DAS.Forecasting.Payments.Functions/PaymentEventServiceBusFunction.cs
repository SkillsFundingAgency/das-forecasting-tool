...
namespace SFA.DAS.Forecasting.Payments.Functions
{
    public class PaymentEventServiceBusFunction : IFunction
	{
	    ...
	    [return: Queue(QueueName.PaymentValidator)]
		    ...
		    TraceWriter writer)
	    {
		    return await FunctionRunner.Run<PaymentEventServiceBusFunction, PaymentEvent>(writer,
			    ...
			    {
				    logger.Info($"Added {nameof(PaymentEvent)} to queue: {QueueName.PaymentValidator},  for EmployerAccountId: {paymentEvent?.EmployerAccountId}");
